...
class PreparedAxisymmetricOperator:
    ...
    def harmonic_geometry_derivatives(
        self,
        object_values: "ArrayLike",
    ) -> tuple["NDArray[np.complexfloating]", "NDArray[np.complexfloating]"]:
        """Return geometry derivatives from the prepared harmonic kernel.

        The radial derivative uses the exact discrete order-shift identity

        ``FFT(cos(beta) * kernel) = 0.5 * (roll(K, 1) + roll(K, -1))``.

        For a sufficiently resolved angular grid this is the sampled form of
        ``J_m'(x) = 0.5 * (J_{m-1}(x) - J_{m+1}(x))``, including the ``i**m``
        phase convention already stored in ``kernel_fft``.  The axial
        derivative reuses ``z_phase``.  Consequently the object is transformed
        once and no coordinate-weighted auxiliary forward solves are needed.

        Compact/truncated implementations must retain at least one additional
        Bessel order beyond the requested output support.  This full discrete
        implementation keeps the complete cyclic spectrum.
        """
        ...
        histogram_fft = self.prepare_object(object_values)
        axial_phase_derivative = (
            1j * self.z_centers[None, :] * self.z_phase
        )
        derivative_q_perp_fourier = np.empty(
            self.data_shape, dtype=self.complex_dtype
        )

        def radial_shift_contraction(
            kernel_slice: "NDArray[np.complexfloating]",
            object_slice: "NDArray[np.complexfloating]",
        ) -> "NDArray[np.complexfloating]":
            return np.einsum(
                "ej,jz,jrh,r,erzh->jh",
                self.form_factors,
        ...
        n_modes = self.phi.size
        if n_modes == 1:
            derivative_q_perp_fourier[:] = 1j * radial_shift_contraction(
                self.kernel_fft, histogram_fft
            )
        else:
            # Keep the large (q, r, mode) kernel in place.  Only contiguous
            # views and (q, mode) contraction outputs are formed; unlike
            # np.roll, this does not copy the complete kernel twice.
            derivative_q_perp_fourier[:, :1] = 0.5j * (
                radial_shift_contraction(
                    self.kernel_fft[:, :, -1:], histogram_fft[..., :1]
                )
                + radial_shift_contraction(
                    self.kernel_fft[:, :, 1:2], histogram_fft[..., :1]
                )
            )
            derivative_q_perp_fourier[:, -1:] = 0.5j * (
                radial_shift_contraction(
                    self.kernel_fft[:, :, -2:-1], histogram_fft[..., -1:]
                )
                + radial_shift_contraction(
                    self.kernel_fft[:, :, :1], histogram_fft[..., -1:]
                )
            ...
            if n_modes > 2:
                derivative_q_perp_fourier[:, 1:-1] = 0.5j * (
                    radial_shift_contraction(
                        self.kernel_fft[:, :, :-2], histogram_fft[..., 1:-1]
                    )
                    + radial_shift_contraction(
                        self.kernel_fft[:, :, 2:], histogram_fft[..., 1:-1]
                    )
                )
        derivative_q_z_fourier = np.einsum(
            "ej,jz,jrh,erzh->jh",
            self.form_factors,
            axial_phase_derivative,
            self.kernel_fft,
        ...
        return (
            np.fft.ifft(derivative_q_perp_fourier, axis=-1).astype(
                self.complex_dtype, copy=False
            ),
            np.fft.ifft(derivative_q_z_fourier, axis=-1).astype(
                self.complex_dtype, copy=False
            ),
        )

    def harmonic_geometry_jacobian_action(
        self,
        object_values: "ArrayLike",
        delta_q_perp: "ArrayLike",
        delta_q_z: "ArrayLike",
    ) -> "NDArray[np.complexfloating]":
        """Apply the recurrence-based geometry JVP to a real perturbation."""
        ...
        derivative_perp, derivative_z = self.harmonic_geometry_derivatives(
            object_values
        )
        return derivative_perp * delta_perp[:, None] + derivative_z * delta_z[:, None]

    def harmonic_geometry_loss_gradient(
        self,
        object_values: "ArrayLike",
        residual: "ArrayLike",
    ) -> tuple["NDArray[np.float64]", "NDArray[np.float64]"]:
        """Apply the recurrence-based real VJP for a complex residual."""
        ...
        gradient_z = np.real(
            np.sum(np.conj(residual_array) * derivative_z, axis=1)
        )
