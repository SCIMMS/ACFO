...
class PreparedSelectedModeDoubleManifoldOperator:
    """Double-manifold operator restricted to selected source azimuthal modes.

    A source proportional to one spherical harmonic ``Y_l^m`` has a single
    complex azimuthal mode ``m``.  A real orbital uses at most the pair
    ``(-m, +m)``.  In the double-manifold expansion only mode pairs satisfying
    ``m_out + m_in`` in ``source_modes`` can then contribute.  This class
    stores only those pairs instead of the full quadratic mode-pair tensor.

    ``forward`` is the full truncated double-manifold operator preceded by an
    exact projection onto ``source_modes``.  ``adjoint_euclidean`` is its
    matched Euclidean adjoint and returns an object in the same modal subspace.
    """
    ...
    def __init__(
        ...
        *,
        ...
        raw_source_modes = np.asarray(source_modes)
        if raw_source_modes.ndim != 1 or raw_source_modes.size == 0:
            raise ValueError("source_modes must be a non-empty integer vector")
        if not np.all(np.isfinite(raw_source_modes)) or not np.all(
            raw_source_modes == np.rint(raw_source_modes)
        ):
            raise ValueError("source_modes must contain only finite integers")
        selected_modes = np.unique(raw_source_modes.astype(np.int64))
        if np.any(np.abs(selected_modes) > 2 * harmonic_cutoff):
            raise ValueError("source_modes must satisfy |mode| <= 2 * harmonic_cutoff")
        self.source_modes = selected_modes

        mode_sum = self.modes[:, None] + self.modes[None, :]
        pair_m, pair_n = np.nonzero(np.isin(mode_sum, self.source_modes))
        if pair_m.size == 0:
            raise ValueError("no retained double-harmonic pair reaches source_modes")
        self.pair_outgoing_indices = pair_m.astype(np.int64)
        self.pair_incident_indices = pair_n.astype(np.int64)
        pair_sums = mode_sum[pair_m, pair_n]
        self.pair_source_indices = np.searchsorted(self.source_modes, pair_sums).astype(
            np.int64
        )
        ...
        self.beta_transform = np.exp(
            -1j * self.source_modes[:, None] * self.beta[None, :]
        )
        ...
        pair_phase = np.power(1j, self.modes[self.pair_outgoing_indices]) * np.power(
            -1j,
            self.modes[self.pair_incident_indices],
        )
        radial_pairs = (
            outgoing_bessel[:, self.pair_outgoing_indices, :][:, None, :, :]
            * incident_bessel[:, self.pair_incident_indices, :][None, :, :, :]
        )
        self.pair_kernel = (
            radial_pairs[..., None]
            * axial[:, :, None, None, :]
            * pair_phase[None, None, :, None, None]
        )
        self.pair_outgoing_angular = np.exp(
            1j
            * self.modes[self.pair_outgoing_indices, None]
            * self.outgoing_phi[None, :]
        )
        self.pair_incident_angular = np.exp(
            1j
            * self.modes[self.pair_incident_indices, None]
            * self.incident_phi[None, :]
        )

        for array in (
            self.source_modes,
            self.pair_outgoing_indices,
            self.pair_incident_indices,
            self.pair_source_indices,
            self.beta_transform,
            self.pair_kernel,
            self.pair_outgoing_angular,
            self.pair_incident_angular,
        ):
            array.setflags(write=False)

    @property
    def retained_pair_count(self) -> int:
        return int(self.pair_source_indices.size)

    @property
    def full_pair_count(self) -> int:
        return int(self.modes.size**2)
    ...
    def project_object(self, object_values: "ArrayLike") -> "NDArray[np.complex128]":
        """Return the exact discrete projection onto the retained beta modes."""
        ...
        return np.einsum(
            "rzs,sb->rzb",
            object_modes / float(self.beta.size),
            np.conj(self.beta_transform),
            optimize=True,
        )

    def forward(self, object_values: "ArrayLike") -> "NDArray[np.complex128]":
        """Apply the selected-source-mode double-manifold forward operator."""
        ...
        pair_object_modes = object_modes[:, :, self.pair_source_indices]
        coefficients = np.einsum(
            "oikrz,rzk->oik",
            self.pair_kernel,
            pair_object_modes,
    ...
    def adjoint_euclidean(
        ...
        """Apply the exact adjoint of the selected-source-mode forward."""

        data = self._data_array(data_values)
        outgoing_projection = np.einsum(
            "oipq,kp->oikq",
            data,
            np.conj(self.pair_outgoing_angular),
            optimize=True,
        )
        coefficient_adjoint = np.einsum(
            "oikq,kq->oik",
            outgoing_projection,
            np.conj(self.pair_incident_angular),
            optimize=True,
        )
        pair_adjoint = np.einsum(
            "oikrz,oik->rzk",
        ...
        object_mode_adjoint = np.zeros(
            (self.r.size, self.z.size, self.source_modes.size),
            dtype=np.complex128,
        )
        for pair_index, source_index in enumerate(self.pair_source_indices):
            object_mode_adjoint[:, :, source_index] += pair_adjoint[:, :, pair_index]
        return np.einsum(
            "rzs,sb->rzb",
            object_mode_adjoint,
            np.conj(self.beta_transform),
