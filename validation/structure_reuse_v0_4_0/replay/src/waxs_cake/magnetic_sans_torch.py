...
class TorchFlatDetectorBandlimitedMagneticSansOperator:
    ...
    def __init__(
        ...
        *,
        ...
        active_r_indices = self.radial_support_indices(
            points,
    ...
    @classmethod
    def radial_support_indices(
        cls,
        points: Any,
        *,
        r_max: float,
        n_r: int,
        source_deposition: str,
    ) -> np.ndarray:
        """Return every radial bin touched by one source geometry.

        The helper shares the exact nearest/linear deposition map used by
        :meth:`configure_source_updates`.  Streamed workloads can therefore
        form a union across all prespecified geometries before constructing a
        compact retained plan.
        """
        ...
        if source_deposition == "nearest":
            radial_indices = (radius * (int(n_r) / float(r_max))).astype(
                np.int64
            )
            np.clip(radial_indices, 0, int(n_r) - 1, out=radial_indices)
            return np.unique(radial_indices).astype(np.intp, copy=False)

        lower, upper, _ = cls._linear_center_indices(
            radius,
            step=float(r_max) / int(n_r),
            size=int(n_r),
            periodic=False,
        )
        return np.unique(np.concatenate([lower, upper])).astype(
            np.intp, copy=False
        )
