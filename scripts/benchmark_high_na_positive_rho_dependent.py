...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Compare signed and positive-only rho-dependent harmonic "
            "Debye-Wolf plans."
        )
    )
    ...
    parser.add_argument("--cutoff-bin-sizes", default="4,8")
    parser.add_argument(
        "--signed-backend",
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("benchmark_results/high_na_positive_rho_dependent.json"),
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=Path("benchmark_results/high_na_positive_rho_dependent.csv"),
    )
    ...
    for case in cases:
        for rho_max in rho_max_values:
            ...
            for cutoff_bin_size in cutoff_bin_sizes:
                ...
                positive_specs = [("positive_rho_packed", False)]
                if args.positive_backend in {"auto", "cpp"}:
                    positive_specs.append(("positive_rho_nocopy", True))
                positive_results = []
                for positive_variant, no_copy_coefficients in positive_specs:
                    positive_plan, positive_build_s, _ = median_time(
                        lambda no_copy_coefficients=no_copy_coefficients: (
                            PreparedPositiveRhoDependentHarmonicDebyeWolfPlan.build(
                                args.nphi,
                                theta,
                                theta_weights,
                                rho_axis,
                                ...
                                cpp_threads=args.cpp_threads,
                                no_copy_coefficients=no_copy_coefficients,
                            )
                        ),
                        args.repeats,
                    )
                    positive_single, positive_single_s, _ = median_time(
                        lambda positive_plan=positive_plan: positive_plan.evaluate(
                            pupil
                        ),
                        args.repeats,
                    )
                    positive_hot, positive_hot_s, _ = median_time(
                        lambda positive_plan=positive_plan: positive_plan.evaluate_many(
                            pupils
                        ),
                        args.repeats,
                    )
                ...
                for variant, plan, build_s, single_s, hot_s, single, hot in variants:
                    row = dict(common)
                    row.update(
                        {
                            "variant": variant,
                            "groups": plan.group_count,
                            "max_signed_modes": plan.used_modes,
                            "mean_signed_modes": plan.mean_used_modes,
                            "mean_stored_abs_modes": None
                            if variant == "signed_rho"
                            else plan.mean_stored_abs_modes,
                            "mode_rho_work": plan.mode_rho_work,
                            "signed_mode_rho_work": None
                            if variant == "signed_rho"
                            else plan.signed_mode_rho_work,
                            ...
                            "build_speedup_vs_signed": speedup(
                                signed_build_s,
                                build_s,
                            ),
                            "single_speedup_vs_signed": speedup(
                                signed_single_s,
                                single_s,
                            ),
    ...
    for row in rows:
        ...
        print(
            ...
            f"{row['variant']:12s} bin={row['cutoff_bin_size']} "
